...
public class PathRedirectHandler : MonoBehaviour
{
    ...
    private void TryExtendPath(RaycastHit hit)
    {
        ...
        if (!GraphMapSetup.Instance.Nodes.TryGetValue(lastNodeId, out LDY_MapNode last) || !last.connectedNodeIds.Contains(node.nodeId))
        {
    ...
    private void UpdatePreviewLine()
    {
        ...
        {
            if (GraphMapSetup.Instance.Nodes.TryGetValue(pathBuilder[i], out LDY_MapNode node))
